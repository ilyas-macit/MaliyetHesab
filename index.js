...
function hesaplaHamMal() {
  ...
  for (let i = 1; i <= 5; i++) {
    const iplikInput = document.querySelector(`#iplik${i}`);
    const fiyatInput = document.querySelector(`#fiyat${i}`);
    const oranInput = document.querySelector(`[name="oran${i}"]`);
    const fireInput = document.querySelector(`[name="fire${i}"]`);
  ...
  for (let i = 1; i <= 5; i++) {
    const oran =
      parseFloat(document.querySelector(`[name="oran${i}"]`).value) || 0;
    oranToplam += oran;
  }
  ...
  // Hesaplama
  for (let i = 1; i <= 5; i++) {
    const fiyat = parseFloat(document.querySelector(`#fiyat${i}`).value) || 0;
    const oran =
      parseFloat(document.querySelector(`[name="oran${i}"]`).value) || 0;
    const fire =
      parseFloat(document.querySelector(`[name="fire${i}"]`).value) || 0;

    let sonuc = fiyat * (oran / 100);
    sonuc += sonuc * (fire / 100);

    const kurSwitch = document.querySelector(`[name="kur${i}"]`);
    if (kurSwitch && kurSwitch.checked) {
      sonuc *= usdTry;
    }
    toplam += sonuc;
  }
  ...
  const boyafireOran = boyafireInput ? parseFloat(boyafireInput.value) || 0 : 0;
  const boyafason = boyafasonInput ? parseFloat(boyafasonInput.value) || 0 : 0;
  ...
  if (bolayiMaliyetUsdId) {
    bolayiMaliyetUsdId.textContent = `${(boyaliMaliyet / usdTry).toFixed(2)}$`;
  }
  if (bolayiMaliyetEuId) {
    bolayiMaliyetEuId.textContent = `${(
      (boyaliMaliyet / usdTry) *
      usdEuro
    ).toFixed(2)}€`;
  }
